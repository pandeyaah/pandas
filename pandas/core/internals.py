...
from pandas.core.common import (_possibly_downcast_to_dtype, isnull,
                                ...
                                is_internal_type, is_array_view, is_array_type)
    ...
    def _astype(self, dtype, copy=False, raise_on_error=True, values=None,
        ...
        try:
            ...
            if values is None:
                ...
                values = com._astype_nansafe(com.ravel_compat(values), dtype, copy=True)
...
class IntNABlock(AbstractIntBlock):
    ...
    def __init__(self, values, **kwargs):
        super(IntNABlock, self).__init__(com.as_nd(values), **kwargs)
    ...
    def _slice(self, slicer):
        ...
        #### TODO could use fancy indexing here
        if isinstance(slicer, slice):
            slicer = nd.range(len(self.values),dtype='int64')[slicer]
        return nd.take(self.values, slicer)
...
class SingleBlockManager(BlockManager):
    ...
    def get_values(self):
        ...
        return com.ndarray_compat(self._block.to_dense(), copy=False)
...
def form_blocks(arrays, names, axes):
    ...
    for i, name_idx in enumerate(names_indexer):
        ...
        if com.is_ndt_type(dtype) and is_integer_dtype(dtype):
            int_items.append((i, k, v))
        elif is_sparse(v):
            sparse_items.append((i, k, v))
        elif issubclass(dtype.type, np.floating):
            float_items.append((i, k, v))
        elif issubclass(dtype.type, np.complexfloating):

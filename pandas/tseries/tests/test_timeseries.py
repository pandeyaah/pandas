...
class TestTimeSeries(tm.TestCase):
    ...
    def test_to_datetime_origin(self):
        ...
        # gh-11276, gh-11745
        # support for julian / alt origin parameters
        ...
        expected = Series(to_datetime(
            julian_dates - Timestamp(0).to_julian_date(), unit='D'))
        assert_series_equal(result, expected)
        ...
        epoch_1960 = Timestamp('1960-01-01')
        epoch_timestamp_convertible = [epoch_1960,
                                       epoch_1960.to_datetime(),
                                       epoch_1960.to_datetime64(),
                                       str(epoch_1960)]

        units_from_epoch = list(range(5)) + ['NaT']

        for unit in units:
            for epoch in epoch_timestamp_convertible:
                expected = Series(
                    [Timedelta(x, unit=unit) +
            ...
            for origin in ['random_string', '13-24-1990']:
                with self.assertRaises(ValueError):
                    to_datetime(units_from_epoch, unit=unit,
                                origin=origin)

        # invalid unit
        with self.assertRaises(ValueError):
            to_datetime([1, 2], unit='D', origin='julian')

...
class TestStyler(TestCase):
    ...
    def test_applymap_subset(self):
        ...
        for slice_ in slices:
            result = self.df.style.applymap(f, subset=slice_)._thunk().ctx
    ...
    def test_bar(self):
        ...
        result = df.style.bar()._thunk().ctx
        expected = {
        ...
        result = df.style.bar(color='red', width=50)._thunk().ctx
        expected = {
            (0, 0): ['width: 10em', ' height: 80%',
                     'background: linear-gradient('
    ...
    def test_highlight_null(self, null_color='red'):
        ...
        result = df.style.highlight_null()._thunk().ctx
        expected = {(0, 0): [''],
    ...
    def test_apply_none(self):
        ...
        result = (pd.DataFrame([[1, 2], [3, 4]])
                  .style.apply(f, axis=None)._thunk().ctx)
    ...
    def test_highlight_max(self):
        ...
        for max_ in [True, False]:
            if max_:
                ...
                df = -df
                attr = 'highlight_min'
            result = getattr(df.style, attr)()._thunk().ctx
            self.assertEqual(result[(1, 1)], ['background-color: yellow'])

            result = getattr(df.style, attr)(color='green')._thunk().ctx
            self.assertEqual(result[(1, 1)], ['background-color: green'])

            result = getattr(df.style, attr)(subset='A')._thunk().ctx
            self.assertEqual(result[(1, 0)], ['background-color: yellow'])

            result = getattr(df.style, attr)(axis=0)._thunk().ctx
            expected = {(1, 0): ['background-color: yellow'],
                        (1, 1): ['background-color: yellow'],
                        (0, 1): [''], (0, 0): ['']}
            self.assertEqual(result, expected)

            result = getattr(df.style, attr)(axis=1)._thunk().ctx
        ...
        df['C'] = ['a', 'b']
        result = df.style.highlight_max()._thunk().ctx
        expected = {(1, 1): ['background-color: yellow']}

        result = df.style.highlight_min()._thunk().ctx
    ...
    def test_background_gradient(self):
        ...
        for axis in [0, 1, 'index', 'columns']:
            for cmap in [None, 'YlOrRd']:
                result = df.style.background_gradient(cmap=cmap)._thunk().ctx

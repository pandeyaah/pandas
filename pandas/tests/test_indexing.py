...
def random_text(nobs=100):
    ...
    idx = np.random.randint(len(letters), size=(nobs,2))
    idx.sort()
    return DataFrame([ [ letters[slice(s,e)] ] for s, e in idx ], columns=['letters'])
...
"""
    ...
    def test_detect_chained_assignment_with_policy(self):

        def check(policy):

            is_column = policy == 'column'
            is_split = policy == 'split'
            ...
            df['A'][0] = -5
            df['A'][1] = np.nan
            expected = DataFrame({ 'A' : Series([-5,np.nan]), 'B' : np.array(np.arange(2,4),dtype=np.float64)})
            assert_frame_equal(df, expected)
            ...
            self.assertRaises(com.SettingWithCopyError, f)
            df['C'][2] = 'foo'
            self.assertEqual(df.loc[2,'C'], 'foo')

        # set the mode.policy to the specified value
        # as a default as well as force chained_asssignment to raise
        for value in ['column','split']:
            with option_context('mode.policy',value,
                                'chained_assignment','raise'):
                check(value)

    def test_detect_chained_assignment(self):

        # this is with the default block policy
